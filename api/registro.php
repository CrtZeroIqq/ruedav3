...
// NUEVO: Campos de networking
$serviciosOfrecidos = sanitize($_POST['servicios_ofrecidos'] ?? '');
$serviciosNecesitados = sanitize($_POST['servicios_necesitados'] ?? '');
...
if (empty($rubro) || strlen($rubro) < 3) {
    $errores[] = 'El rubro es obligatorio';
}
...
try {
    ...
    // Verificar que el dominio del email corporativo no esté ya registrado
    // Excluir dominios públicos de correo gratuito
    $dominio = substr(strrchr($email, "@"), 1);
    $dominiosPublicos = [
        'gmail.com', 'hotmail.com', 'outlook.com', 'yahoo.com', 'icloud.com',
        'live.com', 'msn.com', 'aol.com', 'zoho.com', 'protonmail.com',
        'mail.com', 'yandex.com', 'gmx.com', 'tutanota.com'
    ];

    if ($dominio && !in_array(strtolower($dominio), $dominiosPublicos)) {
        $stmt = $pdo->prepare("SELECT nombre, email_contacto FROM empresas WHERE email_contacto LIKE ?");
        $stmt->execute(['%@' . $dominio]);
        $empresaExistente = $stmt->fetch();
        if ($empresaExistente) {
            $pdo->rollBack();
            jsonResponse(false, 'Ya existe una empresa registrada con el dominio corporativo @' . $dominio . ' (' . $empresaExistente['email_contacto'] . '). Los dominios corporativos solo pueden registrarse una vez.');
        }
    }
    
    // Insertar empresa CON los nuevos campos de networking
    $stmt = $pdo->prepare("
