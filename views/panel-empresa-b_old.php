...
$pageTitle = "Panel Pyme - " . SITE_NAME;

// Verificar acceso
checkAccess([ROL_EMPRESA_B]);

// Obtener datos de la empresa (Pyme)
...
try {
    ...
    // Obtener empresas grandes disponibles
    $stmt = $pdo->query("
        SELECT * FROM empresas 
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8 animate-fade-in">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Panel de Pyme</h1>
            <p class="text-gray-600">Explora empresas grandes y solicita reuniones</p>
        </div>
...
<div id="tab-agenda" class="tab-content hidden">
    ...
    try {
        $stmt = $pdo->prepare("
            ...
            FROM reuniones r
            INNER JOIN bloques_horarios bh ON r.bloque_id = bh.id
            INNER JOIN empresas ea ON bh.empresa_a_id = ea.id
            WHERE r.empresa_b_id = ?
...
function mostrarAgenda(bloques, empresaId) {
    ...
    bloques.forEach(bloque => {
        ...
        if (disponible) {
            ...
            contenidoBoton = `<button onclick="solicitarReunion(${bloque.id}, ${empresaId})" 
        ...
        html += `
            <div class="${claseEstado} border rounded-lg p-4 flex justify-between items-center transition-all duration-200">
...
function solicitarReunion(bloqueId, empresaId) {
    const notas = prompt('Mensaje para la empresa (opcional):');
    ...
    formData.append('empresa_a_id', empresaId);
